...
abstract class Give_Shortcode_Generator {
	...
	protected function generate_docs_link( $field ) {
		...
		$return_array = array(
			'type' => 'container',
			'html' => sprintf(
				'<p class="give-docs-link" style="%5$s">%1$s<span class="dashicons dashicons-editor-help" style="%2$s"></span></p>',
				$field['text'],
				$dashicon_style,
				$a_style,
				isset($field['link']) ? esc_url($field['link']) : '',
				$p_style
			),
		);
		
		if (isset($field['link'])) {
			$return_array['html'] = sprintf(
				'<p class="give-docs-link" style="%5$s"><a href="%4$s" style="%3$s" target="_blank">%1$s<span class="dashicons dashicons-editor-help" style="%2$s"></span></a></p>',
				$field['text'],
				$dashicon_style,
				$a_style,
				esc_url($field['link']),
		...
		return $this->generate_container($return_array);
