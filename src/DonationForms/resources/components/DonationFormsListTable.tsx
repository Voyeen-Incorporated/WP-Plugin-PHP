...
const donationFormsBulkActions: Array<BulkActionsConfig> = [
  {
    ...
    action: async (selected) => {
      const authorSelect = document.getElementById(
        "giveDonationFormsTableSetAuthor"
      ) as HTMLSelectElement;
      const author = authorSelect.value;
      const statusSelect = document.getElementById(
        "giveDonationFormsTableSetStatus"
      ) as HTMLSelectElement;
    ...
    confirm: (selected, names) => (
      <>
        ...
        <div className={styles.flexRow}>
          <label htmlFor="giveDonationFormsTableSetAuthor">
            {__("Set form author", "give")}
          </label>
          <Select
            id="giveDonationFormsTableSetAuthor"
        ...
        <div className={styles.flexRow}>
          <label htmlFor="giveDonationFormsTableSetStatus">
            {__("Set form status", "give")}
          </label>
          <Select
            id="giveDonationFormsTableSetStatus"
            style={{ paddingInlineEnd: "2rem" }}
          >
  ...
  },
  {
    ...
    value: "delete",
    type: "danger",
    isVisible: (data, parameters) =>
      parameters.status === "trash" || !data?.trash,
    action: async (selected) =>
      await API.fetchWithArgs("/delete", { ids: selected.join(",") }, "DELETE"),
    confirm: (selected, names) => (
      <div>
  ...
  },
  {
    ...
    isVisible: (data, parameters) =>
      parameters.status !== "trash" && data?.trash,
    action: async (selected) =>
      await API.fetchWithArgs("/trash", { ids: selected.join(",") }, "DELETE"),
    confirm: (selected, names) => (
      <div>
  ...
  return (
    ...
    >
      {!!window.GiveNextGen?.newFormUrl && (
        <a
          href={window.GiveNextGen.newFormUrl}
      ...
      <a
        href={"post-new.php?post_type=give_forms"}
        className={styles.addFormButton}
      >
        {__("Add Form test", "give")}
      </a>
      {/* This is javed Edit */}
        <button style="donationStatus:nonel"
          className={styles.addFormButton}
          onClick={showLegacyDonationForms}
        >
          {__("Switch to Legacy View test")}
        </button>
    </ListTablePage>
  );
}
...
// };
// // /wp-admin/edit.php?post_type=give_forms